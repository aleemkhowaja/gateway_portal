			...
			var imalIndex = mainJson["IMAL_DMS_INDEX"];
				for (i = 0; i < imalIndex.length; i++)
				{
				    ...
				    for(j = 0; j < dmsIndex.length; j++)
					{
				    	...
				    	if(indexDet.type == 1 || indexDet.type == 3)
				    	{
			...
			$.ajax({
				...
				success : function(data) {
							...
							&& data["theURL"] != undefined && data["theURL"] != '')
					{
						...
						if(data["dmsType"] == "3")
						{
							var appLoadDivId = "appLoad_div_" + pageRef;
							if(!$('#'+appLoadDivId).length || typeof(window.AppLauncher)=="undefined")
							{
								var appLoadDiv = $("<div id='" + appLoadDivId + "'/>");
								appLoadDiv.css("padding", "0");
								$('body').append(appLoadDiv);
								
								document.getElementById("appLoad_div_"+pageRef).innerHTML = "<object name='AppLauncher' style='display:none' id='AppLauncher' classid='"+data["appLnchrCLSID"]+"' codebase='../login/PathAppLauncher.cab#version="+data["appLnchrVersion"]+"'></object>";
							}
							AppLauncher.LaunchCommandLineApp(data["theURL"],data["theArgs"]);
							_showProgressBar(false);
						}
						else if ((location.protocol == 'https:' && data["theURL"].indexOf('https:') === -1)

				 	...
				 	if(elemntTagName !== "label" && !buttonElement && !collapCont)
					{
				 		...
				 		else if(settingsRight)
						{
							...
							if(theElement.hasClass("ui-jqgrid"))
							{
								...
								var _iconToolTip = "";
								if(_grdCaption!=null && _grdCaption.trim()!="" && _grdCaption!="undefined")
								{
									_iconToolTip = for_key+" "+_grdCaption;
								}
								else
								{
									if(theElement.parents("div.liveSearchOverlay").length >= 1)
									{
										_iconToolTip = for_key+" "+live_search_key;
									}
									else
									{
...
//	}	
	...
	if(relId.indexOf("gbox_") > -1)
	{
		...
		{
			//checking if this method is called for live search
			if($(theCustIconDiv).parents("div.liveSearchOverlay").length >= 1)
			{
				_addToTitle = for_key+" "+live_search_key;
			}else{
	...
	buttons.push({text:saveLabel,id:"fieldCust_saveBtn",click:function(){
		 ...
		 /*953614 Specify Grid Filter Query Expression - Grid/Livesearch Customization Enhancements*/
			 fillFilterExpInputFromTA();
			 validateFilterExpression(relId,function(){
	...
	$(theCustIconDiv).parents("div.liveSearchOverlay").hide();
...
/*953614 Specify Grid Filter Query Expression - Grid/Livesearch Customization Enhancements*/
/*filter-expression */
//convert user-column-title filter expression query to database-query and assign the value to the input to be sent
function fillFilterExpInputFromTA()
{
	var gridId = $('#objectId').val();
	var $gridTable = $("#"+gridId + "_" + custScrPageRef);
	var filterExpressionValue = $("#fldcust_filterExpression_"+custScrPageRef).val().trim();
	if(typeof filterExpressionValue === "string" && filterExpressionValue.length > 0)
	{
		var valToSend = convertColumnTitlesToPropNames(filterExpressionValue, $gridTable);
		$("#fldcust_filterExpressionToSend_"+custScrPageRef).val(valToSend);
	}
	else
	{
		$("#fldcust_filterExpressionToSend_"+custScrPageRef).val("");
	}
}

/**
 * used in opening object customization page for live_search from field customization page
 * @param elemId input id related to this live_search
 * @returns
 */
function openObjectCustPage(elemId)
{
	var configIcon =  document.getElementById(elemId.replace("lookuptxt","config_icon_gbox_gridtab"));
	// customization(advanced options -> settings) has been called after live-search grid was constructed, hence,
	//target the config_icon of that live_search grid object
	if(isNotNullAndNotUndefined(configIcon)) 
	{
		openObjectCustomization(configIcon);
	}
	// customization has been called before live-search grid was constructed, that means
	// live_search grid object's config_icon doesn't exist
	else
	{
		var tempSpan = document.createElement("span"); 
		tempSpan.id = elemId.replace("lookuptxt","temp_span");
		tempSpan.setAttribute("relId" , elemId.replace("lookuptxt","gbox_gridtab"));
		tempSpan.setAttribute("relName" , "undefined");
		
		var $overlay = $("#"+elemId.replace("lookuptxt","overlay"));
		$overlay.append(tempSpan);
		openObjectCustomization(tempSpan);
		$overlay.children("#"+tempSpan.id).remove();
	}
	$("#global_elem_cust_div").dialog("close");
}

/**
 * hides the button that opens object customization for live-search grid related to textField with txtFieldId
 * if and only if the former grid wasn't constructed before opening the field customization page containing
 * the former button.
 * @param txtFieldId - id of textField input of a live-search
 * @returns
 */
function manageHidingLvSrchBtn(txtFieldId)
{
	if(isNotEmptyString(fromLiveSearch) && fromLiveSearch === "1")
	{
		var table = document.getElementById(txtFieldId.replace("lookuptxt","gridtab"));
		if(isNullOrUndefined(table) || isNullOrUndefined(table.p))
		{
			var $objCustButton = $("#"+"live_search_op_cust_from_"+txtFieldId);
			$objCustButton.hide();
		}
	}
}
...
/**
 * validates filter expression if existed, then calls callback if validation passes
 * @param gridObjectId
 * @param callback - basically the save process has to be the callback
 * @returns
 */
function validateFilterExpression(gridObjectId,callback)
{
	if(!($("#fldcust_filterExpressionToSend_"+custScrPageRef).val() === ""))
	{
		var dataToSend = {};
		dataToSend[$("#fldcust_filterExpressionToSend_"+custScrPageRef).attr("name")]=$("#fldcust_filterExpressionToSend_"+custScrPageRef).val();
		dataToSend["_pageRef"] =  $("[name = 'custCO.cutomizationPROG_REF']").val();
		dataToSend["fltrExpValidateFlg"] = "1";
		dataToSend["userVisibleFilterExpression"] = $("#fldcust_filterExpression_"+custScrPageRef).val();
		validateFltrExpAgainstEngine(dataToSend,function(data) {
			validateFltrExpAgainstDB(gridObjectId,data, callback);
		});
	}
	else
	{
	...
	/*
	 * first step of validation: check validity using the engine against given filter expression
	 * and retrieve the filter query after parsing and executing on server side
	 */
	function validateFltrExpAgainstEngine(dataToSend, callback)
	{
		$.ajax({
			 url: jQuery.contextPath+"/path/objectCustomization/objectCustomizationMain_validateFilterExpression",
	        type:"post",
			 dataType:"json",
			 data: dataToSend,
			 success: function(data)
	...
	/* second step of validation: if first step was executed normally
	 * query real data from server and check if the query succeed
	 * with given filter expression
	 * */
	function validateFltrExpAgainstDB(gridObjectId,data, callback)
	{
		$grid = $("#"+gridObjectId.replace('gbox_',''));
		var gridData = $.extend({},$grid.jqGrid("getGridParam","postData"));
		gridData.filterExpToValidate = data.filterExpToValidate;
		gridData.fltrExpValidateFlg = data.fltrExpValidateFlg;
		gridData.userVisibleFilterExpression = $("#fldcust_filterExpression_"+custScrPageRef).val();
		loadDataUrl = $grid.jqGrid("getGridParam").url;
		$.ajax({
			url: loadDataUrl,
			type:"post",
			dataType:"json",

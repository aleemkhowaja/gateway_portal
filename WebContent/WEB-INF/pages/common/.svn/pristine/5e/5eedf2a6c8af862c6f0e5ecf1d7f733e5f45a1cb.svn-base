			...
			<tr>
				<td>
					...
					<tr>
						...
						<ps:checkbox id="checkbox_fldcust_filterExpression_${_pageRef}"  valOpt="1:0"
								         name="custCO.hasFilterExpr"
								         onclick="objCust_filterExpCheckBoxClicked(this)" />
								         
							<ps:label key="filter_expression_key"
								id="lbl_fldcust_filterExpression_${_pageRef}"
								for="checkbox_fldcust_filterExpression_${_pageRef}">
							</ps:label>
							
						</td>
						<td width="10%"></td>
						<td >
							<ps:textarea id="fldcust_filterExpression_${_pageRef}"
								name="filterExpressionUIOnly"
								cols="50"
								>
							</ps:textarea>
							<ps:hidden id="fldcust_filterExpressionToSend_${_pageRef}" name="custCO.sysParamObjDisplayVO.FILTER_EXPR"></ps:hidden>
						</td>
					</tr>
					</table>
				</td>
...
$(document).ready(function(){
  initializefilterExp('fldcust_filterExpression_<ps:property value="_pageRef" escapeHtml="false" escapeJavaScript="true"/>' , expression_cust_tags);
});
